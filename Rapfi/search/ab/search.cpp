...
// VCN (Victory by Continue N) search functions
// N=4: VCF (continuous four threats)
// N=3: VC3 (continuous three threats, defender can pass up to 3 times)
// N=2: VC2 (continuous two threats, defender can pass up to 4 times)
template <Rule Rule, NodeType NT, int N>
Value vcnsearch(Board &board, SearchStack *ss, Value alpha, Value beta, int passCount = 0, Depth depth = 0.0f);
template <Rule Rule, NodeType NT, int N>
...
void ABSearcher::searchMain(MainSearchThread &th)
{
    ...
    // VCN mode: pure mate solver for VC2/VC3
...
void ABSearcher::search(SearchThread &th)
{
    ...
    for (sd.rootDepth = startDepth; sd.rootDepth <= maxDepth && !th.threads.isTerminating();
        ...
        if (!options.isAnalysisMode() && !th.threads.main()->inPonder && !rmp.enabled()) {
            // In VCN mode, only stop when we find a mate (winning VCN)
            if (options.isVCNEnabled()) {
                // In VCN mode, continue searching until we find a mate
                // or until max depth is reached
                if (isMate && sd.rootDepth - firstMateDepth >= Config::NumIterationAfterMate)
                    th.threads.stopThinking();
            }
            else if (isMate && sd.rootDepth - firstMateDepth >= Config::NumIterationAfterMate)
                th.threads.stopThinking();
...
/// ============================================================================
/// VCN (Victory by Continue N) Search Implementation
/// ============================================================================
///
/// Strict VCN Definition:
/// A side can VCN-win if and only if it can force a win and the opponent
/// cannot pass (6-N) times before the final five-in-a-row.
///
/// - VC4 (VCF): continuous four threats, opponent cannot pass 2 times
/// - VC3: continuous three+ threats, opponent can pass up to 3 times
/// - VC2: continuous two+ threats, opponent can pass up to 4 times
///
/// The pass mechanism works as follows:
/// - When defending, the defender can choose to pass (if passCount < 6-N)
/// - After a pass, the attacker gets another move
/// - The pass count tracks how many times the defender has passed

namespace {

/// Maximum allowed passes for each VCN level
constexpr int maxPassesForVCN(int n) { return 6 - n; }

/// Minimum Pattern4 level required for attack in VCN search
...
/// Check if a Pattern4 is a valid attack move for given VCN level
inline bool isValidVCNAttack(Pattern4 p4, int n)
{
    if (p4 >= B_FLEX4)
        return true;  // Four or Five is always valid attack
    return p4 >= minAttackPattern4ForVCN(n);
}

}  // namespace

/// ============================================================================
/// Pure VCN (Victory by Continue N) Mate Solver
/// ============================================================================
/// 
/// VCN is a PURE mate solver, NOT a Q-Search! It should NOT use static evaluation.
/// 
/// Return values:
/// - VALUE_MATE + ply: Attack side wins (forced mate found)
/// - VALUE_MATED - ply: Attack side loses (no valid attack moves)
/// - Draw value: Only when board is full or max moves reached
///
/// The solver searches until it finds a forced win or exhausts all possibilities.

/// The VCN search function for attack side.
/// @tparam N VCN level (2, 3, or 4)
/// @param passCount Number of times defender has already passed
template <Rule Rule, NodeType NT, int N>
Value vcnsearch(Board &board, SearchStack *ss, Value alpha, Value beta, int passCount, Depth depth)
{
    constexpr bool PvNode = NT == PV || NT == Root;
    constexpr int  MaxPass = maxPassesForVCN(N);
    ...
    // Step 3. Check for immediate winning (five in a row)
    if (board.p4Count(self, A_FIVE)) {
        // Found a five - this is a win
    ...
    // VCN barrier (0x61C8864680B583EBULL) ensures VCN entries never collide with normal search
    HashKey posKey  = board.zobristKey()
                    ^ HashKey(0x61C8864680B583EBULL)   // VCN exclusive barrier
                    ^ HashKey(N * 0x3141592653589793ULL)  // VCN level (2 or 3)
                    ^ HashKey(passCount * 0x9E3779B97F4A7C15ULL);  // Pass count
    Value   ttValue = VALUE_NONE;
    Value   ttEval  = VALUE_NONE;
    bool    ttIsPv  = false;
    Bound   ttBound = BOUND_NONE;
    ...
    // Use high depth reference for VCN mate scores
    int vcnRefDepth = (int)DEPTH_QVCF + 50;
    ...
    // Step 6. Generate attack moves
    ScoredMove moveList[MAX_MOVES];
    ScoredMove *end;

    if constexpr (N == 3) {
        // VC3: generate H_FLEX3 and above
        end = generate<VCF | VCT>(board, moveList);
    }
    else if constexpr (N == 2) {
        // VC2: generate L_FLEX2 and above
        end = generate<VCF | VCT | VC2>(board, moveList);
    }
    else {
        // N==4 is VCF, handled separately
        end = generate<VCF>(board, moveList);
    }

    // Step 7. If no valid attack moves, attack fails (return mated)
    if (moveList == end) {
        // No valid VCN attack moves - attack side loses
        Value result = mated_in(ss->ply);
        TT.store(posKey, result, VALUE_NONE, PvNode, BOUND_UPPER, Pos::NONE,
                 (int)DEPTH_QVCF + 50, ss->ply);
        return result;
    }
    ...
    // If no move was found valid, bestValue is still -INF, meaning mated
    ...
    TT.store(posKey, bestValue, VALUE_NONE, PvNode, bound, bestMove, 
             (int)DEPTH_QVCF + 50, ss->ply);
...
/// @param passCount Number of times defender has already passed
/// 
/// VCN Defence Logic (Pure Mate Solver):
/// Defender tries to survive by:
/// 1. Blocking opponent's threats
/// 2. Counter-attacking (if possible)
/// 3. Passing (if quota remains)
/// 
/// Returns:
/// - VALUE_MATE + ply: Defender gets mated (attack side wins)
/// - VALUE_MATED - ply: Defender survives (attack side fails)
template <Rule Rule, NodeType NT, int N>
Value vcndefend(Board &board, SearchStack *ss, Value alpha, Value beta, int passCount, Depth depth)
{
    ...
    if (oppo5) {
        ...
        // For renju, if black's defence move is a forbidden point, black loses
        if (Rule == Rule::RENJU && self == BLACK && board.checkForbiddenPoint(move)) {
    ...
    // Step 5. Generate defence moves
    // Use ALL to generate all candidate points (generate<VCF|VCT> only checks defender's patterns)
    ScoredMove moveList[MAX_MOVES];
    ScoredMove *end = generate<ALL>(board, moveList);

    // Sort moves by Pattern4
    std::sort(moveList, end, [&board, self](const ScoredMove &a, const ScoredMove &b) {
        return board.cell(a.pos).pattern4[self] > board.cell(b.pos).pattern4[self];
    });

    // Step 6. Search defence moves
    for (ScoredMove *sm = moveList; sm != end; ++sm) {
        Pos move = *sm;
        ...
        // Only consider moves that have defensive value
    ...
    // Step 7. Try passing (if we still have pass quota)
...
template Value vcnsearch<Rule::FREESTYLE, PV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::FREESTYLE, PV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::FREESTYLE, NonPV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::FREESTYLE, NonPV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::STANDARD, PV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::STANDARD, PV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::STANDARD, NonPV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::STANDARD, NonPV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::RENJU, PV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::RENJU, PV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::RENJU, NonPV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcnsearch<Rule::RENJU, NonPV, 3>(Board &, SearchStack *, Value, Value, int, Depth);

template Value vcndefend<Rule::FREESTYLE, PV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcndefend<Rule::FREESTYLE, PV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcndefend<Rule::FREESTYLE, NonPV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcndefend<Rule::FREESTYLE, NonPV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcndefend<Rule::STANDARD, PV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcndefend<Rule::STANDARD, PV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcndefend<Rule::STANDARD, NonPV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcndefend<Rule::STANDARD, NonPV, 3>(Board &, SearchStack *, Value, Value, int, Depth);
template Value vcndefend<Rule::RENJU, PV, 2>(Board &, SearchStack *, Value, Value, int, Depth);
